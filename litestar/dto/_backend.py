...
class DTOBackend:
    ...
    def __init__(
        ...
        self.annotation = build_annotation_for_backend(
            model_type, field_definition, self.transfer_model_type
        )
    ...
    def parse_model(
        ...
        """
        defined_fields = []
        generic_field_definitons = list(
            FieldDefinition.from_annotation(model_type).generic_types or ()
        )
        ...
        for field_definition in dto_factory.generate_field_definitions(model_type):
            if field_definition.is_type_var:
                ...
                field_definition = replace(
                    field_definition,
                    annotation=base_arg_field.annotation,
                    raw=base_arg_field.raw,
                )

            if _should_mark_private(
                field_definition, dto_factory.config.underscore_fields_private
            ):
    ...
    def _create_transfer_model_name(self, model_name: str) -> str:
        ...
        if (
            short_name := f"{short_name_prefix}{model_name}{name_suffix}"
        ) not in self._seen_model_names:
            name = short_name
        elif (
            long_name := f"{long_name_prefix}{model_name}{name_suffix}"
        ) not in self._seen_model_names:
            name = long_name
        else:
    ...
    def parse_raw(
        ...
        if request_encoding == RequestEncodingType.MESSAGEPACK:
            result = decode_msgpack(
                value=raw, target_type=self.annotation, type_decoders=type_decoders
            )
        else:
            result = decode_json(
                value=raw, target_type=self.annotation, type_decoders=type_decoders
    ...
    def populate_data_from_builtins(
        self, builtins: Any, asgi_connection: ASGIConnection
    ) -> Any:
        ...
        return self.transfer_data_from_builtins(
            self.parse_builtins(builtins, asgi_connection)
        )
    ...
    ) -> CompositeTypeHandler | None:
        if field_definition.is_union:
            return self._create_union_type

        if field_definition.is_tuple:
            if (
                len(field_definition.inner_types) == 2
                and field_definition.inner_types[1].annotation is Ellipsis
            ):
    ...
    def _create_transfer_type(
        ...
        if composite_type_handler := self._get_handler_for_field_definition(
            field_definition
        ):
        ...
        if self._dto_factory(field_definition.annotation).detect_nested_field(
            field_definition
        ):
            ...
            transfer_model = NestedFieldInfo(
                model=self.create_transfer_model_type(
                    unique_name, nested_field_definitions
                ),
    ...
    def _create_collection_type(
        ...
        inner_type = self._create_transfer_type(
            field_definition=inner_types[0]
            if inner_types
        ...
        return CollectionType(
            ...
            has_nested=inner_type.has_nested,
        )
    ...
    def _create_mapping_type(
        ...
        value_type = self._create_transfer_type(
            field_definition=inner_types[1]
            if inner_types
            else FieldDefinition.from_annotation(Any),
            exclude=exclude,
            include=include,
...
def _filter_nested_field(
    field_name_set: AbstractSet[str], field_name: str
) -> AbstractSet[str]:
...
def _filter_nested_field_mapping(
    field_name_mapping: Mapping[str, str], field_name: str
) -> dict[str, str]:
    """Filter a nested field name."""
    return {
        split[1]: v
...
def _transfer_nested_union_type_data(
    ...
    for inner_type in transfer_type.inner_types:
        if isinstance(inner_type, CompositeType):
            raise RuntimeError(
                "Composite inner types not (yet) supported for nested unions."
            )

        if inner_type.nested_field_info and isinstance(
            source_value,
            inner_type.nested_field_info.model
            if is_data_field
...
def _create_struct_for_field_definitions(
    ...
    for field_definition in field_definitions:
        ...
        field_type = _create_transfer_model_type_annotation(
            field_definition.transfer_type
        )
        if field_definition.is_partial:
    ...
    return defstruct(
        model_name, struct_fields, frozen=True, kw_only=True, rename=rename_strategy
    )
...
def _should_mark_private(
    field_definition: DTOFieldDefinition, underscore_fields_private: bool
) -> bool:
...
def _should_exclude_field(
    ...
    if (
        include
        and field_name not in include
        and not (any(f.startswith(f"{field_name}.") for f in include))
    ):
        return True
...
def _create_transfer_model_tuple_type(transfer_type: TupleType) -> Any:
    inner_types = tuple(
        _create_transfer_model_type_annotation(t) for t in transfer_type.inner_types
    )
